...
from config import BOT_TOKEN, CREDENTIALS_PATH, SPREADSHEET_ID
from services.storage_service import storage, JSONStorage
from services.gsheets_service import GoogleSheetsManager
from middlewares import RoleCheckMiddleware
from handlers import start, booking, schedule, admin, common
...
async def main():
    ...
    gsheets_manager = GoogleSheetsManager(CREDENTIALS_PATH, SPREADSHEET_ID)
    if gsheets_manager.connect():
        storage.set_gsheets_manager(gsheets_manager)
        logger.info("Google Sheets manager initialized successfully")
    else:
        logger.warning("Failed to initialize Google Sheets manager")
